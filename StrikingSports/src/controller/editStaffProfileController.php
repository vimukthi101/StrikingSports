									...
									if(preg_match('/^(\d{10})|(^$)$/',$phone)){
										...
										if(mysqli_query($con, $updateEmployee)){
											...
											if(mysqli_query($con, $updateEmployeeAddress)){
												$updateEmployeeContact = "UPDATE staff SET contact_no='".$contactNo."' WHERE email='".$_SESSION['email']."'";
												if(mysqli_query($con, $updateEmployeeContact)){
													header('Location:../staffProfile.php?error=su');
												} else {
													//redirect to form query faile
													header('Location:../editStaffProfile.php?error=qf');	
												}
											} else {
												//redirect to form query faile
												header('Location:../editStaffProfile.php?error=qf');	
										...
										//contact no
										header('Location:../editStaffProfile.php?error=wp');
									}
								} else {
									//address city
									header('Location:../editStaffProfile.php?error=wc');
								}
							} else {
								//address lane
								header('Location:../editStaffProfile.php?error=wa');
							}
						} else {
							//addres no
							header('Location:../editStaffProfile.php?error=wn');
						}
					} else {
						//last name is invalid
						header('Location:../editStaffProfile.php?error=wl');
					}
				} else {
					//middle name is invalid
					header('Location:../editStaffProfile.php?error=wm');
				}
			} else {
				//first name is invalid
				header('Location:../editStaffProfile.php?error=wf');
			}
		} else {
			//redirect to form empty fields
			header('Location:../editStaffProfile.php?error=ef');	
		}
	} else {
